...
Current preferred path: commit public screenshot files under `screenshots/` to GitHub. Railway's Docker build includes that folder, and the entrypoint copies it into the persistent volume when `USE_STORAGE_SYMLINKS=1`.
